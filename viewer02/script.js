...
function discoverComponents(gltf) {
  ...
  model.traverse(node => {
    ...
    const configured = COMPONENT_INFO[index];
    const info = configured?.sourceName === sourceName ? configured : null;
    const component = {
      id: components.length, node, sourceName,
      name: info?.displayName || sourceName,
      description: info?.description || `${info?.displayName || sourceName} in the water leak detection assembly.`,
...
function buildComponentTree() {
  ...
  function appendNode(node, parentList) {
    ...
    const bottomIndex = children.findIndex(child => child.userData.componentLabel === 'Bottom Assembly');
    if (bottomIndex !== -1 && children.some(child => child.userData.componentLabel === 'Top Assembly')) {
      const [bottom] = children.splice(bottomIndex, 1);
      children.splice(children.findIndex(child => child.userData.componentLabel === 'Top Assembly') + 1, 0, bottom);
    }
    ...
    if (part) {
      ...
      summary.textContent = node.userData.componentLabel || node.name;
